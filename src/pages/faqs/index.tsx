  ...
  return (
    <div>
      <div className='grid grid-row md:grid-cols-2 lg:grid-cols-2 gap-3 w-full lg:w-[80%] m-auto mt-5 lg:mt-10 mb-4 p-3'>
        <div>
          <h3 className='font-bold mb-4'>Purchases & Refunds</h3>
          <Accordion defaultValue="" variant="contained">
            {
              faqs.filter((val) => val.header === 'Purchases & Refunds').map(({ content, header, title }) => (
        ...
        <div>
          <h3 className='font-bold mb-4'>Making Courses</h3>
          <Accordion defaultValue="" variant="contained">
            {
              faqs.filter((val) => val.header === 'Making Courses').map(({ content, header, title }) => (
                <Accordion.Item key={title} value={title}>
                  <Accordion.Control>{title}</Accordion.Control>
                  <Accordion.Panel>{content}</Accordion.Panel>
                </Accordion.Item>
      ...
      <div className='mt-10'>
        ...
        >
          <div className='flex flex-col md:flex-row lg:flex-row justify-between text-white gap-4 lg:gap-0 md:items-center lg:items-center w-full lg:w-[80%] m-auto font-bold uppercase mt-[20px] md:mt-[30px] lg:mt-[50px]'>
            <h3 className='md:w-[40%] lg:w-[30%] md:text-[20px] lg:text-[20px]'>If You Have Any Questions You Can Call Me 24/7</h3>
            <Link href={''} className='md:text-[20px] lg:text-[20px]'>Call us now</Link>
          </div>
        </div>
        <div className='relative top-[-200px] lg:top-[-250px]'>
          ...
          <div className='grid grid-row md:grid-cols-3 lg:grid-cols-3 items-cente w-full lg:w-[80%] m-auto shadow-lg'>
            <div className='col-span-3 md:col-span-1 lg:col-span-1 bg-white p-8 lg:rounded-l-lg'>
              <h3 className='font-bold text-[18px] mb-2 '>Take the First Step</h3>
              <p className='text-wrap text-gray-400 text-[14px]'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
              {
                content.map(({ title, desc }, id) => (
                  <div className='flex gap-3 mt-3' key={id}>
                    <h1 className='text-[50px] text-gray-300 font-medium hover:text-purple-600 hover:cursor-default'>{`0${id+1}`}</h1>
                    <div className='mt-4'>
