...
const About = () => {
  const content = [
    {
      icon: (
        <WifiSquare size='70' color='#81D170' className='block m-auto mb-3' />
      ),
      total: 94532,
    ...
    {
      icon: (
        <KeyboardOpen size='70' color='#4489F3' className='block m-auto mb-3' />
      ),
      total: 11223,
      ...
      <div className=' w-full lg:w-[80%] m-auto mb-[50px]'>
        ...
        <div className='flex flex-col md:flex-row lg:flex-row justify-evenly gap-3 m-auto p-3 text-center'>
          {content.map(({ icon, total, title }, id) => (
            <div
              className={`about bg-white rounded-md shadow-md p-4 w-full lg:w-[250px] h-[250px] md:first:mt-10 lg:first:mt-10 md:last:mt-10  lg:last:mt-10 hover:border-b-4 first:hover:border-b-[#81D170] last:hover:border-b-[#ED7459] first:text-[#81D170] last:text-[#ED7459] second:text-blue-500 cursor-pointer`}
              key={id}>
              <div className='mt-6'>
                {icon}
                <div>
