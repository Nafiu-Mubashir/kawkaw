...
const Contact = () => {
  const content = [
    {
      icon: (
        <Location size='70' color='#4489F3' className='block m-auto mb-3' />
      ),
    ...
    <div className='w-full lg:w-[80%] m-auto'>
      <div className='flex flex-col md:flex-row lg:flex-row justify-evenly gap-3 m-auto p-3 text-center mt-5 lg:mt-10 mb-4'>
        {content.map(({ icon, description, title }, id) => (
          <div
            className={` bg-white rounded-md shadow-md p-4 w-full lg:w-[350px] h-[250px] md:h-[300px] hover:border-b-4 first:hover:border-b-[#4489F3] last:hover:border-b-mine hover:even:border-b-[#81D170] cursor-pointer`}
            key={id}>
            <div className='mt-6'>
              {icon}
              <div>
                <p className='font-bold  text-[14px] lg:text-[18px]'>{title}</p>
                <p>{description}</p>
              </div>
            </div>
          </div>
      ...
      <div className='mb-4 p-3'>
        <h3 className='font-bold text-center  text-[20px] lg:text-[24px] lg:w-[50%] m-auto mb-3'>
      ...
      <div className='mt-5 mb-3 p-3'>
        <iframe
          width='100%'
          height='600'
          src='https://maps.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q=bank%20of%20industry,%20Abuja,%20Nigeria+(KAWKAW)&amp;t=&amp;z=14&amp;ie=UTF8&amp;iwloc=B&amp;output=embed'>
